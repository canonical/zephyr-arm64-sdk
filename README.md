# Zephyr arm64 Toolchain SDK for Workshop

This SDK provides the AArch64 cross-compiler toolchain for building Zephyr
firmware targeting the aarch64-zephyr-elf target triple. Use this SDK alongside
...
toolchain bundle and the aarch64-zephyr-elf cross-compiler connected.
...
2. Connect the `arm64` plug on `zephyr-sdk-ng` to the `toolchain` slot on this SDK.
...
- Interface: `mount`
- Workshop source: `$SDK`
- Purpose: Provides the aarch64-zephyr-elf cross-compiler to the `zephyr` base
  SDK.
